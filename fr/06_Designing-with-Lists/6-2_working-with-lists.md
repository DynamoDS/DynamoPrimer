...
Toutes les opérations répertoriées ci-dessus possèdent des noeuds Dynamo similaires qui permettent de travailler avec des listes de données génériques. Les leçons ci-dessous présentent certaines des opérations fondamentales que vous pouvez effectuer sur des listes.
...
2. Connectez-le à l'entrée *x* d'un noeud *Point.ByCoordinates*.
3. Connectez le noeud de l'étape précédente à l'entrée d'origine d'un noeud *Plane.ByOriginNormal*.
4. À l'aide d'un noeud *Cercle.ByPlaneRadius*, connectez le noeud de l'étape précédente à l'entrée plane.
5. À l'aide d'un*bloc de code*, désignez une valeur de ```50;``` pour le *rayon*. Il s'agit du premier cercle créé.
6. À l'aide d'un noeud *Geometry.Translate*, déplacez le cercle de 100 unités vers le haut dans la direction Z.
7. À l'aide d'un noeud de *bloc de code*, définissez un intervalle de dix nombres entre 0 et 1 avec la ligne de code suivante : ```0..1..#10;```
8. Connectez le bloc de code de l'étape précédente à l'entrée *param* de deux noeuds *Curve.PointAtParameter*. Connectez *Circle.ByPlaneRadius* à l'entrée curve du noeud supérieur, puis *Geometry.Translate* à l'entrée curve du noeud situé en dessous.
9. À l'aide d'un noeud *Line.ByStartPointEndPoint*, connectez les deux noeuds *Curve.PointAtParameter*.

![Exercice](images/6-2/Exercise/38.jpg)

> 1. Un noeud *Watch3D* affiche les résultats de *Line.ByStartPointEndPoint*. Vous dessinez actuellement des lignes entre deux cercles pour représenter les opérations de liste de base et utiliserez ce graphique Dynamo de base pour parcourir les actions de liste ci-dessous.

### List.Count

![Nombre](images/6-2/count.jpg)

> Le noeud *List.Count* est simple : il compte le nombre de valeurs dans une liste et renvoie ce nombre. Ce noeud devient plus subtil lorsque vous travaillez avec des listes de listes, mais vous le découvrirez dans les sections suivantes.
...
> 1. Le noeud *List.Count* renvoie le nombre de lignes dans le noeud *Line.ByStartPointEndPoint*. La valeur est de 10 dans ce cas, ce qui correspond au nombre de points créés à partir du noeud *Code Block* d'origine.
...
> 1. À l'aide du noeud *List.GetItemAtIndex*, sélectionnez l'index *"0"* ou le premier élément de la liste de lignes.
2. Le noeud *Watch3D* indique que vous avez sélectionné une ligne. Remarque : pour obtenir l'image ci-dessus, veillez à désactiver l'aperçu de *Line.ByStartPointEndPoint*.
...
2. Insérez un noeud *List.Reverse* entre *Curve.PointAtParameter* et *Line.ByStartPointEndPoint* pour l'un des points de la liste.
3. Les noeuds *Watch3D* affichent deux résultats différents. Le premier affiche le résultat sans liste inversée. Les lignes se connectent verticalement aux points voisins. Toutefois, la liste inversée connecte tous les points à l'ordre opposé dans l'autre liste.

### List.ShiftIndices

![maj](images/6-2/shift.jpg)

> *List.ShiftIndices* est un bon outil pour créer des torsions ou des motifs hélicoïdaux, ou toute autre manipulation de données similaire. Ce noeud déplace les éléments d'une liste d'un certain nombre d'index.
...
3. Le changement est subtil, mais toutes les lignes du noeud *Watch3D* inférieur se sont déplacées d'un index lors de la connexion à l'autre jeu de points.
...
> 1. À l'aide d'un *bloc de code*, définissez une expression avec la syntaxe : ```0..List.Count(list);```. Connectez le noeud *Curve.PointAtParameter* à l'entrée *list*. Vous allez découvrir davantage cette configuration dans le chapitre relatif aux blocs de code, mais la ligne de code dans ce cas présente une liste représentant chaque index du noeud *Curve.PointAtParameter*.
2. À l'aide d'un noeud *"%"* (module), connectez la sortie du *bloc de code* à l'entrée *x* et une valeur de *4* à l'entrée *y*. Vous obtiendrez ainsi le reste lors de la division de la liste d'index par 4. Le module est un noeud très utile pour la création de réseaux. Toutes les valeurs sont lues comme étant les restes possibles de 4 : 0, 1, 2, 3.
3. Grâce au noeud *module*, vous savez qu'une valeur de 0 signifie que l'index est divisible par 4 (0,4,8, etc.). À l'aide d'un noeud *"=="*, vous pouvez tester la visibilité en effectuant un test sur une valeur de *"0"*.
4. Le noeud *Watch* montre que vous avez un motif true/false qui indique : *true,false,false,false...*.
5. À l'aide de ce motif true/false, connectez-vous à l'entrée mask de deux noeuds *List.FilterByBooleanMask*.
6. Connectez le noeud *Curve.PointAtParameter* à chaque entrée list de *List.FilterByBooleanMask*.
7. La sortie de *Filter.ByBooleanMask* indique *"in"* et *"out"*. L'expression *"in"* représente les valeurs qui avaient une valeur de masque *"true"* et l'expression *"out"*" représente les valeurs qui avaient la valeur *"false"*. En connectant les sorties *"in"* aux entrées *startPoint* et *endPoint* d'un noeud *Line.ByStartPointEndPoint*, vous avez créé une liste filtrée de lignes.
8. Le noeud *Watch3D* indique que vous avez moins de lignes que de points. Vous n'avez sélectionné que 25 % des noeuds en filtrant uniquement les valeurs réelles.
