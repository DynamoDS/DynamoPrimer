...
2. Joignez les deux courbes dans une polycourbe en connectant le composant *List.Create* à un noeud *Polycurve.ByJoiningCurves*.
3. Créez une liste pour les deux courbes au bas de la façade.
4. Joignez les deux courbes dans une polycourbe en connectant le composant *List.Create* à un noeud *Polycurve.ByJoiningCurves*.
...
3. Connectez le *bloc de code* à l'entrée *param* d'un noeud *"Curve.PlaneAtParameter"* et connectez l'arête supérieure à l'entrée *curve*. Vous obtiendrez ainsi dix plans, répartis uniformément sur l'ensemble de la façade.
...
> 1. À l'aide du noeud *Geometry.Intersect* (notez la combinaison du produit vectoriel), connectez le noeud *Curve.PlaneAtParameter* à l'entrée *entity* du noeud *Geometry.Intersect*. Connectez le noeud principal *List.Create* à l'entrée *geometry*. Les points sont maintenant affichés dans la fenêtre Dynamo. Ils représentent l'intersection de chaque courbe avec les plans définis.
...
> 1. Connectez le noeud *Geometry.Intersect* à l'entrée de liste de *List.Map*.
2. Connectez un noeud *Flatten* à l'entrée f(x) de *List.Map*. Les résultats donnent 3 listes, chacune contenant un nombre égal au nombre de fermes.
...
4. Connectez le fichier *List.Map* à un noeud *List.Transpose*. Vous disposez à présent de la sortie de données souhaitée.
5. Pour confirmer que les données sont correctes, ajoutez un noeud *Polygon.ByPoints* dans la zone de dessin et vérifiez deux fois l'aperçu Dynamo.

![Exercice](images/8-4/Exercise/03.jpg)

> Mettez en réseau les composants adaptatifs de la même façon que vous avez créé les polygones.

> 1. Ajoutez un noeud *AdaptiveComponent.ByPoints* à la zone de dessin et connectez le noeud *List.Transpose* à l'entrée *points*.
2. À l'aide d'un noeud *Family Types*, sélectionnez la famille *"AdaptiveTruss"* et connectez-la à l'entrée *familySymbol* du noeud *AdaptiveComponent.ByPoints*.
...
> 1. En zoomant et en vous déplaçant vers la droite dans l'exemple de graphique, vous apercevez un grand groupe de noeuds : il s'agit d'opérations géométriques qui génèrent la structure de toiture de treille visible dans l'aperçu Dynamo. Ces noeuds sont générés à l'aide de la fonctionnalité *Noeud vers code*, comme décrit dans la [section de bloc de code](../07_Code-Block/7-2_Design-Script-syntax.md#Node) du guide.
...
> 1. Lorsque vous déposez le noeud *DirectShape.ByGeometry* sur la zone de dessin, vous voyez qu'il comporte quatre entrées : **geometry, category, material** et **name**.
2. La géométrie sera le solide créé à partir de la partie de création de géométrie du graphique
3. L'entrée de catégorie est sélectionnée à l'aide du noeud déroulant *Categories*. Dans ce cas, vous allez utiliser "Ossature".
4. Vous pouvez sélectionner l'entrée de matériau via le réseau de noeuds ci-dessus, bien qu'il puisse être plus simplement défini comme "Par défaut" dans ce cas.
