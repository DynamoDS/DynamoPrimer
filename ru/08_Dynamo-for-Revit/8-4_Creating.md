...
> 1. Используя узел *Geometry.Intersect* (обратите внимание на режим переплетения Cross Product), встройте компонент *Curve.PlaneAtParameter* в набор входных данных *entity* узла *Geometry.Intersect*. Встройте основной узел *List.Create* в набор входных данных *geometry*. Теперь на видовом экране Dynamo можно увидеть точки, обозначающие пересечение кривых с заданными плоскостями.
