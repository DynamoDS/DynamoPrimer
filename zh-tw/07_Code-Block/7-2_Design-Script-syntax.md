...
節點的交織與程式碼區塊的交織稍有不同。對於節點，使用者在節點上按一下右鍵，然後選取要執行的交織選項。對於代碼區塊，使用者針對資料的構建方式會有更多的控制。代碼區塊速寫方法使用*複製指南*來設定多個一維清單應採用的配對方式。角括號「<>」中的數字會定義所產生巢狀清單的階層：<1>、<2>、<3> 等。![鑲邊](images/7-2/lacing.jpg)</3></2></1>
...
> Dynamo 已經為視覺圖表、交織與全部項目自動建立一個文字版本。在您的視覺腳本上測試這一點，並釋放代碼區塊的強大功能！
